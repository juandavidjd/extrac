#!/usr/bin/env python3
"""
ORDEN MAESTRA v4 - Pipeline con Fuzzy Image Matching
...
OUTPUT_PATH = '/opt/odi/data/orden_maestra_v4'
                        ...
                        try:
                            with open(csv_path, 'r', encoding=enc) as file:
                                ...
                                for row in reader:
                                    ...
                                    img_url = (row.get('Imagen_Externa') or row.get('imagen') or
                                              row.get('URL_IMAGEN') or row.get('image_url') or
        ...
        try:
            with open(main_csv, 'r', encoding=enc) as file:
                ...
                for row in reader:
                    ...
                    if not img:
                        url_field = (row.get('Imagen_Externa') or row.get('imagen') or
                                    row.get('URL_IMAGEN') or row.get('image_url') or '')
...
def process_vision_store(store_name):
    ...
    for item in data:
        ...
        sku = item.get('sku') or item.get('codigo') or f'{store_name}-{len(products)+1}'
        price = parse_price_cop(item.get('price') or item.get('precio') or 0)
...
def main():
    ...
    print('='*60)
    print('ORDEN MAESTRA v4 - FUZZY IMAGE MATCHING')
    print('='*60)
    print()
