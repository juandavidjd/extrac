#!/usr/bin/env python3
"""
ORDEN MAESTRA v6 - CSV Priority Fix + Fuzzy + Vision
...
OUTPUT_PATH = '/opt/odi/data/orden_maestra_v6'
...
def process_csv_store(store_name, store_path):
    ...
    # Buscar CSV principal - priorizar Base_Datos_ sobre catalogo_imagenes_
    main_csv = None
    for subdir in ['catalogo', 'precios', '.']:
        dir_path = os.path.join(store_path, subdir) if subdir != '.' else store_path
        if not os.path.exists(dir_path):
            continue
        csvs = [f for f in os.listdir(dir_path) if f.lower().endswith('.csv')]
        # Prioridad 1: Base_Datos_
        for f in csvs:
            if 'base' in f.lower() and 'imagen' not in f.lower():
        ...
        for f in csvs:
            if 'producto' in f.lower() and 'imagen' not in f.lower():
                main_csv = os.path.join(dir_path, f)
                break
        if main_csv:
            break
        # Prioridad 3: cualquier CSV que no sea imagenes
        for f in csvs:
            if 'imagen' not in f.lower():
                main_csv = os.path.join(dir_path, f)
                break
        if main_csv:
            break
...
def main():
    ...
    print('='*60)
    print('ORDEN MAESTRA v6 - CSV PRIORITY + FUZZY + VISION')
    print('='*60)
    print()
